...
use tungstenite::error::{UrlError, Error};
use tungstenite::http::{Uri};
use tungstenite::handshake::client::{Request, generate_key};
use super::commands::{SubscribeParams, WebsocketCommand, WebsocketCommandParams, WebsocketCommandResponse};
...
impl IntoClientRequest for WebsocketMarketDataRequest{
    fn into_client_request(self) -> tungstenite::error::Result<Request> {
        ...
        let authority = endpoint_uri.authority().ok_or(Error::Url(UrlError::NoHostName))?.as_str();
        let host = authority
            .find('@')
...
impl WebsocketMarketDataClient{
    pub async fn start(&self, message_sink: Sender<WebsocketMarketDataMessage>){
        ...
        let (mut websocket_connection, _) = connect_async(ws_request.clone()).await.expect("Failed to connect");
        ...
        websocket_connection.send(Message::Text(serde_json::to_string(&websocket_command).unwrap())).await.unwrap();

        let command_response: WebsocketCommandResponse = serde_json::from_str(&websocket_connection.next().await.unwrap().unwrap().into_text().unwrap()).unwrap();

        match command_response {
            WebsocketCommandResponse::SubscribeError(v) => {println!("{:?}", v);} // TODO: add proper handling
            WebsocketCommandResponse::SubscribeSuccessful(v) => {println!("{:?}", v);}
            _ => {}
        }
